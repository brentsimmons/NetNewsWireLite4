...
- (void)dealloc {
	[initialRequestedURL release];
	[lastRequestedURL release];
	[lastLoadedURL release];
...
- (void)keyDown:(NSEvent *)event {
	...
	if (!RSIsEmpty(s)) {
		...
		if (ch != ' ' && !self.skipKeystrokes) {
			if ([[NSApp delegate] respondsToSelector:@selector(handleKeyStroke:inView:)]) {		
				if ([[NSApp delegate] handleKeyStroke:event inView:self])
					return;
				}
			}
		}
