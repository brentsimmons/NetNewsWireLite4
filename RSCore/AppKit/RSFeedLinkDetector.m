...
+ (void)initialize {
    _linkCache = [NSMutableDictionary dictionaryWithCapacity:100];
    }
...
+ (NSString *)_filterURL:(NSString *)urlString {
    ...
    if ([urlString rs_contains:@"/sub/http"]) {
        ...
        if (!RSIsEmpty(d)) {
            ...
            if (!RSIsEmpty(tempString)) {
                urlString = (NSString *)CFBridgingRelease(CFURLCreateStringByReplacingPercentEscapes(kCFAllocatorDefault, (CFStringRef)tempString, (CFStringRef)@"%20"));
            }
        }
    }
