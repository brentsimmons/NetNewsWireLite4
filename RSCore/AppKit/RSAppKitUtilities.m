...
void RSHelpGotoPage(NSString *page) {
    _registerHelpBookIfNeeded();
    AHGotoPage((__bridge CFStringRef)[[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleHelpBookName"], (__bridge CFStringRef)page, nil);
}
...
OSStatus RSLaunchAppWithPathSync(NSString *f) {
    ...
    launchSpec.appURL = (__bridge CFURLRef)[NSURL fileURLWithPath:f];
...
NSString *RSPathToDefaultAppForURLScheme(NSString *urlScheme) {
    ...
    LSGetApplicationForURL((__bridge CFURLRef)[NSURL URLWithString:urlScheme], kLSRolesAll, nil, &appURL);
