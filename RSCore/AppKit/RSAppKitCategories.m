...
+ (NSColor *)rs_grayBorderColor {
	...
	if (!grayBorderColor)
		grayBorderColor = [[NSColor colorWithCalibratedWhite:0.75 alpha:1.0] retain];
...
+ (NSColor *)rs_darkGrayBorderColor {
	...
	if (!darkGrayBorderColor)
		darkGrayBorderColor = [[NSColor colorWithCalibratedWhite:0.36 alpha:1.0] retain];
...
+ (NSColor *)rs_borderColor {
	...
	if (!borderColor)
		borderColor = [RSGray(0.25) retain];
	return borderColor;
}
...
+ (void)rs_setCoolBlueColor:(NSColor *)color {
	[coolBlueColor autorelease];
	coolBlueColor = [color retain];
}
...
+ (NSMenuItem *)rs_menuItemWithTitle:(NSString *)aString action:(SEL)aSelector target:(id)target
	...
	return [[[self alloc] rs_initWithTitle:aString action:aSelector target:target
					  representedObject:obj tag:tag] autorelease];
}


+ (NSMenuItem *)rs_menuItemWithTitle:(NSString *)aString action:(SEL)aSelector target:(id)target {
	NSMenuItem *item = [[[self alloc] rs_initWithTitle:aString action:aSelector] autorelease];
	if (!item)
		return nil;
...
- (void)rs_setDrawAsEnabled:(BOOL)fl {
	if (fl)
		...
		[self setAttributedStringValue:[[[NSAttributedString alloc] initWithString:[self stringValue] attributes:atts] autorelease]];
	}	
}
