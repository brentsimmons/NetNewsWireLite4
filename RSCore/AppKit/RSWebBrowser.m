...
+ (NSArray *)pluginSuffixes {    
    ...
    if (!pluginSuffixes)
        pluginSuffixes = [NSArray arrayWithObjects:@".mp3", @".au", @".snd", @".mid", @".midi", @".aiff", @".avi", @".mov", nil];
...
NSString *RSPathToDefaultWebBrowser(void) {
    CFURLRef appURL ;
    LSGetApplicationForURL((__bridge CFURLRef)[NSURL URLWithString:@"http:"], kLSRolesAll, nil, &appURL);
