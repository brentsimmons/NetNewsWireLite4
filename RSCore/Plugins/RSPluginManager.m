...
@property (nonatomic, retain, readwrite) NSArray *sharingCommands;
@property (nonatomic, retain) id<RSPluginHelper> pluginHelper;
@property (nonatomic, retain) NSMutableArray *pluginClasses;
@property (nonatomic, retain) NSMutableArray *appObserverPlugins;
@property (nonatomic, retain) NSMutableArray *adManagerPlugins;
@end
...
- (id)init {
	...
	appObserverPlugins = [[NSMutableArray array] retain];
#if TARGET_OS_IPHONE
...
- (void)dealloc {
	[plugins release];
	[commands release];
	[sharingCommands release];
	[pluginHelper release];
	[pluginClasses release];
	[appObserverPlugins release];
	[adManagerPlugins release];
	[super dealloc];
}
...
- (void)registerPluginOfClass:(Class)pluginClass {
	...
	id<RSPlugin> plugin = [[[pluginClass alloc] init] autorelease];
...
- (void)addPluginCommand:(id<RSPluginCommand>)aPluginCommand toMenu:(NSMenu *)aMenu associatedObject:(id)anAssociatedObject indentationLevel:(NSInteger)indentationLevel {
	...
	NSMenuItem *aMenuItem = [[[NSMenuItem alloc] initWithTitle:title action:@selector(performSharingPluginCommandWithSender:) keyEquivalent:@""] autorelease];
...
- (void)addLabelForPlugin:(id<RSPlugin>)aPlugin toMenu:(NSMenu *)aMenu {
	if ([aPlugin respondsToSelector:@selector(titleForGroup)])
		[aMenu addItem:[[[NSMenuItem alloc] initWithTitle:aPlugin.titleForGroup action:nil keyEquivalent:@""] autorelease]];
}
