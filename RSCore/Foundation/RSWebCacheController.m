...
@property (nonatomic, strong, readonly) NSString *baseCacheFolder;
@property (nonatomic, strong, readonly) NSString *cacheFolderName;
...
- (id)init {
    if (![super init])
        return nil;
    baseCacheFolder = NSTemporaryDirectory();
    cacheFolderName = RSWebCacheFolderName;
    return self;
}
...
- (RS_PLATFORM_IMAGE *)cachedImageAtURL:(NSURL *)URL {
    ...
    if (imageData == nil)
        return nil;
    return [[RS_PLATFORM_IMAGE alloc] initWithData:imageData];
}
...
- (id)init {
    ...
    baseCacheFolder = [paths objectAtIndex:0];
    cacheFolderName = RSPermanentWebCacheName;
    return self;
}
