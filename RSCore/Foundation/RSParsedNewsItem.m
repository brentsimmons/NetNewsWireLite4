...
- (void)dealloc {
	...
	[authorEmail release];
	[authorURL release];
	[googleOriginalID release];
	[originalSourceName release];
	[originalSourceURL release];
...
- (NSData *)hashOfReadOnlyAttributes {
	...
	hashOfReadOnlyAttributes = [[NSData rs_md5HashWithString:s] retain];
	return hashOfReadOnlyAttributes;
}
...
- (NSString *)plainTextTitle {
	...
	plainTextTitle = [[self.title rs_stringByMakingPlainTextTitle] retain];
	if (RSStringIsEmpty(plainTextTitle))
		plainTextTitle = [[self.preview rs_stringByMakingPlainTextTitle] retain];
	return plainTextTitle;
}
...
- (NSString *)preview {
	...
	if (!RSStringIsEmpty(previewSource)) {
		...
		strippedPreview = [[RSStringWithDecodedEntities(strippedPreview) mutableCopy] autorelease];
