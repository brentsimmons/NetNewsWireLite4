...
@property (nonatomic, retain, readwrite) NSArray *articles;
@end
...
- (void)updateArticles:(NSArray *)articlesToForceInclusion {
	...
	if (!RSIsEmpty(feedURLStrings) && !RSStringIsEmpty(accountID))
		combinedArrayOfArticles = [[[RSDataArticle sortedArticlesForFeedsWithURLs:feedURLStrings accountID:accountID sortDescriptor:sortDescriptor moc:moc] mutableCopy] autorelease];
	...
	for (RSFeed *oneFeed in feedsToFetch) {
		...
		if ([oneFeed respondsToSelector:@selector(fetchArticlesWithSortDescriptor:moc:)]) {
			...
			if (RSIsEmpty(combinedArrayOfArticles))
				combinedArrayOfArticles = [[oneFeedArticles mutableCopy] autorelease];
			else
	...
	if (!RSIsEmpty(unreadFeedURLStrings) && !RSStringIsEmpty(accountID)) {
		...
		if (RSIsEmpty(combinedArrayOfArticles))
			combinedArrayOfArticles = [[unreadArticles mutableCopy] autorelease];
		else
