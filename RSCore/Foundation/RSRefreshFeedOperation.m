...
- (void)saveArticles:(NSArray *)articles {
	...
	RSLocalAccountCoreDataArticleSaverOperation *localAccountCoreDataArticleSaverOperation = [[[RSLocalAccountCoreDataArticleSaverOperation alloc] initWithParsedArticles:articles feedURL:self.feedURL accountIdentifier:self.accountIdentifier] autorelease];
...
- (void)parseAndProcessFeed {
	...
	self.parser = [[[RSFeedParserProxy alloc] init] autorelease];
	...
	if (![self isCancelled]) {
		if (feedParser.feedHomePageURL != nil || feedParser.feedTitle != nil) {
			RSParsedFeedInfo *feedInfo = [[[RSParsedFeedInfo alloc] init] autorelease];
