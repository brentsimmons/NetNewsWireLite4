...
@property (nonatomic, retain, readwrite) NSMutableArray *feeds;
@property (nonatomic, retain) NSMutableDictionary *feedsDictionary;
@property (nonatomic, retain, readwrite) NSMutableArray *folders;
@property (nonatomic, retain, readwrite) RSTreeNode *accountTreeNode;
@property (nonatomic, retain) NSTimer *unreadCountTimer;
@property (nonatomic, assign, readwrite) NSUInteger unreadCount;
@property (nonatomic, retain) NSTimer *totalUnreadCountTimer;
@property (nonatomic, retain) NSTimer *saveAccountTimer;
...
- (id)initWithAccountType:(RSAccountType)aType login:(NSString *)aLogin {
	...
	if (aType == RSAccountTypeLocal)
		identifier = [RSDataAccountIdentifierOnMyMac retain];
	else if (aType == RSAccountTypeGoogleReader && !RSStringIsEmpty(aLogin))
		identifier = [RSDataAccountIdentifierForGoogleReaderWithLogin(aLogin) retain];
	feeds = [[NSMutableArray array] retain];
	feedsDictionary = [[NSMutableDictionary dictionary] retain];
...
- (void)dealloc {
	...
	[unreadCountTimer release];
	[totalUnreadCountTimer rs_invalidateIfValid];
	[totalUnreadCountTimer release];
	[saveAccountTimer rs_invalidateIfValid];
	[saveAccountTimer release];
	[[NSNotificationCenter defaultCenter] removeObserver:self];
	[identifier release];
	[login release];
	[title release];
	[feeds release];
	[feedsDictionary release];
	[pathToPlist release];
	[super dealloc];
}
...
- (NSString *)pathToPlist {
	...
	pathToPlist = [[rs_app_delegate.pathToDataFolder stringByAppendingPathComponent:filename] retain];
...
- (void)saveToDiskInBackgroundIfNeeded {
	if (self.needsToBeSavedOnDisk) {
		...
		NSInvocationOperation *saveOperation = [[[NSInvocationOperation alloc] initWithTarget:self selector:@selector(saveDictionaryToDiskInBackground:) object:dictionaryRepresentation] autorelease];
...
- (void)addArchivedItems:(NSArray *)archivedItems toTreeNode:(RSTreeNode *)aTreeNode {
	...
	for (id oneItem in archivedItems) {
		...
		if ([oneItem isKindOfClass:[NSDictionary class]]) {
			RSFolder *oneFolder = [[[RSFolder alloc] initWithDiskDictionary:oneItem inAccount:self] autorelease];
...
- (RSFolder *)addFolderWithName:(NSString *)folderName {
	...
	if ([self folderWithNameExists:folderName])
		return nil;
	RSFolder *aFolder = [[[RSFolder alloc] initWithName:folderName account:self] autorelease];
...
- (void)loadFeedsFromAccountDictionary:(NSDictionary *)accountDictionary {
	...
	for (NSDictionary *oneFeedDictionary in feedsFromDisk) {
		RSFeed *oneFeed = [[[RSFeed alloc] initWithDiskDictionary:oneFeedDictionary inAccount:self] autorelease];
...
//	NSMutableArray *URLsOfFeedsWithInvalidUnreadCount = [NSMutableArray array];
	for (RSFeed *oneFeed in self.feeds) {
		...
		RSFeedCountUnreadOperation *countUnreadOperation = [[[RSFeedCountUnreadOperation alloc] initWithFeedURL:oneFeed.URL accountID:self.identifier delegate:self callbackSelector:@selector(feedCountUnreadOperationDidComplete:)] autorelease];
