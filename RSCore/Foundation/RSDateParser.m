...
static NSUInteger indexOfFirstSpace (const char *bytes, NSUInteger numberOfBytes) {
	/*This is used by the Twitter date parser. It's probably at index 3, after the weekday abbreviation.
	 We can check quickly. Returns NSNotFound if not found.*/
	if (numberOfBytes > 4) {
		if (bytes[0] != spaceCharacter && bytes[1] != spaceCharacter && bytes[2] != spaceCharacter && bytes[3] == spaceCharacter)
			return 3;
	}
	NSUInteger i;
	for (i = 0; i < numberOfBytes; i++) {
		if (bytes[i] == spaceCharacter)
			return i;
	}
	return NSNotFound;
}
...
static NSUInteger indexOfLastSpace(const char *bytes, NSUInteger numberOfBytes) {
	NSInteger i;
	for (i = (NSInteger)numberOfBytes - 1; i >= 0; i--) {
		if (bytes[i] == spaceCharacter)
			return (NSUInteger)i;
	}
	return NSNotFound;
...
static NSDate *RSParseTwitterTimelineDateWithBytes(const char *bytes, NSUInteger numberOfBytes) {
	...
	finalIndex = indexOfFirstSpace(bytes, numberOfBytes);
	...
	minute = nextNumericValue(bytes, numberOfBytes, finalIndex + 1, 2, &finalIndex);
	second = nextNumericValue(bytes, numberOfBytes, finalIndex + 1, 2, &finalIndex);
	timeZoneOffset = parsedTimeZoneOffset(bytes, numberOfBytes, finalIndex + 1);
	finalIndex = indexOfLastSpace(bytes, numberOfBytes);
	year = nextNumericValue(bytes, numberOfBytes, finalIndex + 1, 4, &finalIndex);
	return dateWithYearMonthDayHourMinuteSecondAndTimeZoneOffset(year, month, day, hour, minute, second, timeZoneOffset);	
}
...
NSDate *RSTwitterTimelineDateWithBytes(const char *bytes, NSUInteger numberOfBytes) {
	if (numberOfBytesIsOutsideReasonableRange(numberOfBytes))
		return nil;
	return RSParseTwitterTimelineDateWithBytes(bytes, numberOfBytes);
}


NSDate *RSTwitterTimelineDateWithString(NSString *twitterDateString) {
	const char *utf8String = [twitterDateString UTF8String];
	return RSTwitterTimelineDateWithBytes(utf8String, strlen(utf8String));	
}
