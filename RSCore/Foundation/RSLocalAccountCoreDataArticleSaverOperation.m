...
@property (nonatomic, retain) NSArray *existingArticles;
@property (nonatomic, retain) NSArray *parsedArticles;
...
@property (nonatomic, retain) NSMutableSet *managedObjectIDsOfItemsInFeed;
...
- (id)initWithParsedArticles:(NSArray *)someParsedArticles feedURL:(NSURL *)aFeedURL accountIdentifier:(NSString *)anAccountIdentifier {
	...
	if (self == nil)
		return nil;
	accountIdentifier = [anAccountIdentifier retain];
	feedURL = [aFeedURL retain];
	parsedArticles = [someParsedArticles retain];
	managedObjectIDsOfItemsInFeed = [[NSMutableSet setWithCapacity:[someParsedArticles count]] retain];
	return self;
}
...
- (void)dealloc {
	...
	[existingArticles release];
	[feedURL release];
	[parsedArticles release];
	[temporaryMOC release];
	[managedObjectIDsOfItemsInFeed release];
	[super dealloc];
}
...
- (NSArray *)existingArticles {
	...
	NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
	existingArticles = [[RSDataArticle articlesForFeedWithURL:self.feedURL accountID:accountIdentifier moc:self.temporaryMOC] retain];
	[pool drain];
	return existingArticles;
}
