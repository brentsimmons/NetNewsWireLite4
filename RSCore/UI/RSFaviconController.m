...
//		NSString *folder = RSCacheFolderForAppSubFolder(RSFaviconCacheFolderName, NO);
	...
	checkedURLs = [[NSMutableSet setWithCapacity:1000] retain];
	dumpCheckedURLsCacheTimer = [[NSTimer scheduledTimerWithTimeInterval:(60 * 60) target:self selector:@selector(dumpCheckedURLsTimerDidFire:) userInfo:nil repeats:YES] retain];
	return self;
}
...
- (void)dealloc {
	...
	[dumpCheckedURLsCacheTimer release];
	[checkedURLs release];
...
- (void)faviconDownloadOperationDidFinish:(RSDownloadOperation *)faviconDownloadOperation {
	...
	NSInvocationOperation *saveFaviconOperation = [[[NSInvocationOperation alloc] initWithTarget:self selector:@selector(saveFaviconWithDictionary:) object:faviconDictionary] autorelease];
...
- (void)downloadFaviconAtURL:(NSURL *)faviconURL {
	RSDownloadOperation *faviconDownloadOperation = [[[RSDownloadOperation alloc] initWithURL:faviconURL delegate:self callbackSelector:@selector(faviconDownloadOperationDidFinish:) parser:nil useWebCache:NO] autorelease];
...
- (CGImageRef)faviconForURL:(NSURL *)faviconURL {
	...
	if (cgImage != nil)
		return cgImage;
	NSInvocationOperation *fetchFaviconOperation = [[[NSInvocationOperation alloc] initWithTarget:self selector:@selector(fetchfavicon:) object:faviconURL] autorelease];
