...
@property (nonatomic, retain) RSCache *cgImageCache;
@property (nonatomic, retain) RSCache *pngDataCache;
...
- (void)dealloc {
	...
	[cgImageCache release];
	[pngDataCache release];
	[super dealloc];
}
...
- (CGImageRef)cachedCGImageForFilename:(NSString *)filename {
	return (CGImageRef)[self.cgImageCache objectForKey:filename];
}
