...
- (CGAffineTransform)transformForOrientation {
  ...
  if (orientation == UIInterfaceOrientationLandscapeLeft) {
    return CGAffineTransformMakeRotation(pi_float*1.5f);
  } else if (orientation == UIInterfaceOrientationLandscapeRight) {
    return CGAffineTransformMakeRotation(pi_float/2.0f);
  } else if (orientation == UIInterfaceOrientationPortraitUpsideDown) {
    return CGAffineTransformMakeRotation(0.0f - pi_float);
  } else {
...
- (void)sizeToFitOrientation:(BOOL)transform {
  ...
  CGFloat width = floorf(scale_factor * frame.size.width) - kPadding * 2;
  CGFloat height = floorf(scale_factor * frame.size.height) - kPadding * 2;
...
- (void)drawRect:(CGRect)rect {
  CGRect grayRect = CGRectOffset(rect, -0.5f, -0.5f);
  [self drawRect:grayRect fill:kBorderGray radius:10];

  CGRect headerRect = CGRectMake(
    ceilf(rect.origin.x + kBorderWidth), ceilf(rect.origin.y + kBorderWidth),
    rect.size.width - kBorderWidth*2.0f, _titleLabel.frame.size.height);
  ...
  CGRect webRect = CGRectMake(
    ceilf(rect.origin.x + kBorderWidth), headerRect.origin.y + headerRect.size.height,
    rect.size.width - kBorderWidth*2.0f, _webView.frame.size.height+1.0f);
...
//	if (!_showingKeyboard && [self shouldRotateToOrientation:orientation]) {
	if ([self shouldRotateToOrientation:orientation]) { //PBS 10/2/2010: rotate even if keyboard is showing
		self.alpha = 0.0f;
    [self updateWebOrientation];

		[self sizeToFitOrientation:YES];
    CGFloat duration = (CGFloat)([UIApplication sharedApplication].statusBarOrientationAnimationDuration) * 5.0f;
    [UIView beginAnimations:nil context:nil];
...
- (void)show {
  ...
  _iconView.frame = CGRectMake(
    ...
    kBorderWidth + floorf(_titleLabel.frame.size.height/2 - _iconView.frame.size.height/2),
  ...
  [self dialogWillAppear];
	...
	//PBS 10/2/2010: knock off the bouncing thing
	self.transform = [self transformForOrientation];
	self.alpha = 0.1f;
