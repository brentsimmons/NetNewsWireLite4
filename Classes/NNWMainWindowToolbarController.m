...
@property (nonatomic, retain) NSMutableArray *pluginCommandIDs;
@property (nonatomic, retain) NSMutableArray *pluginCommands;
@property (nonatomic, retain) NSToolbar *toolbar;
@property (nonatomic, retain) NSMenuItem *shareMenuRepresentation;
...
- (void)dealloc {
	[actionPopupButton release];
	[addRemoveSegmentedControl release];
	[articleThemePopupButton release];
	[pluginCommandIDs release];
	[pluginCommands release];
	[refreshButton release];
	[refreshButtonContainerView release];
	[refreshProgressIndicator release];
	[shareMenuRepresentation release];
	[sharePopupButton release];
	[toolbar release];
	[window release];
	[super dealloc];
}
...
- (void)setupToolbar {
	...
	self.toolbar = [[[NSToolbar alloc] initWithIdentifier:@"MainWindowToolbar"] autorelease];
	...
	self.shareMenuRepresentation = [[[NSMenuItem alloc] initWithTitle:NSLocalizedString(@"Share", @"Share menu") action:nil keyEquivalent:@""] autorelease];
	NSMenu *shareSubmenu = [[[NSMenu alloc] initWithTitle:NSLocalizedString(@"Share", @"Share menu")] autorelease];
...
- (NSMutableArray *)pluginCommands {
	...
	pluginCommands = [tempArray retain];
	return pluginCommands;
}
...
- (NSMutableArray *)pluginCommandIDs {
	...
	pluginCommandIDs = [tempArray retain];
	return pluginCommandIDs;
}


- (NSToolbarItem *)toolbarItemForPluginCommand:(id<RSPluginCommand>)aPluginCommand {
	NSToolbarItem *toolbarItem = [[[NSToolbarItem alloc] initWithItemIdentifier:aPluginCommand.commandID] autorelease];
...
- (NSToolbarItem *)toolbar:(NSToolbar *)aToolbar itemForItemIdentifier:(NSString *)itemIdentifier willBeInsertedIntoToolbar:(BOOL)flag {
	...
	if ([itemIdentifier isEqualToString:NNWToolbarItemRefresh]) {
		NSToolbarItem *toolbarItem = [[[NSToolbarItem alloc] initWithItemIdentifier:NNWToolbarItemRefresh] autorelease];
	...
	if ([itemIdentifier isEqualToString:NNWToolbarItemArticleTheme]) {
		NNWArticleThemeToolbarItem *toolbarItem = [[[NNWArticleThemeToolbarItem alloc] initWithItemIdentifier:NNWToolbarItemArticleTheme] autorelease];
...
//	}
	...
	if ([itemIdentifier isEqualToString:NNWToolbarItemAddRemove]) {
		NSToolbarItem *toolbarItem = [[[NSToolbarItem alloc] initWithItemIdentifier:NNWToolbarItemAddRemove] autorelease];
	...
	if ([itemIdentifier isEqualToString:NNWToolbarItemShare]) {
		NSToolbarItem *toolbarItem = [[[NSToolbarItem alloc] initWithItemIdentifier:NNWToolbarItemShare] autorelease];
	...
	if ([itemIdentifier isEqualToString:NNWToolbarItemNextUnread]) {
		NSToolbarItem *toolbarItem = [[[NSToolbarItem alloc] initWithItemIdentifier:NNWToolbarItemNextUnread] autorelease];
	...
	if ([itemIdentifier isEqualToString:NNWToolbarItemMarkAllAsRead]) {
		NSToolbarItem *toolbarItem = [[[NSToolbarItem alloc] initWithItemIdentifier:NNWToolbarItemMarkAllAsRead] autorelease];
	...
	if ([itemIdentifier isEqualToString:NNWToolbarItemNewFolder]) {
		NSToolbarItem *toolbarItem = [[[NSToolbarItem alloc] initWithItemIdentifier:NNWToolbarItemNewFolder] autorelease];
...
- (void)addStylesToMenu:(NSMenu *)aMenu {
	...
	for (NSString *oneStyleSheetName in [NNWStyleSheetController sharedController].styleSheetNames) {
		NSMenuItem *oneStyleSheetItem = [[[NSMenuItem alloc] initWithTitle:oneStyleSheetName action:@selector(switchToStyleSheet:) keyEquivalent:@""] autorelease];
...
- (NSMenuItem *)menuFormRepresentation {
	...
	if (!self.menuItem)
		self.menuItem = [[[NSMenuItem alloc] initWithTitle:NSLocalizedString(@"Article Style", @"Command") action:nil keyEquivalent:@""] autorelease];

	NSMenu *submenu = [[[NSMenu alloc] initWithTitle:NSLocalizedString(@"Article Style", @"Command")] autorelease];
