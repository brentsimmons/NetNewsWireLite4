...
@property (nonatomic, retain) NNWInstapaperCredentialsEditor *instapaperCredentialsEditor;
...
@property (nonatomic, retain, readwrite) id<RSSharableItem> sharableItem;
...
- (id)initWithSharableItem:(id<RSSharableItem>)aSharableItem pluginHelper:(id<RSPluginHelper>)aPluginHelper callbackTarget:(id)aCallbackTarget callbackSelector:(SEL)aCallbackSelector {
	...
	if (self == nil)
		return nil;
	sharableItem = [aSharableItem retain];
	pluginHelper = [aPluginHelper retain];
	callbackTarget = [aCallbackTarget retain]; //yes, because app may dispose of the plugin command
...
- (void)dealloc {
	...
	[instapaperCredentialsEditor release];
