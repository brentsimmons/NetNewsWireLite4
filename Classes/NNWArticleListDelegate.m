...
@property (nonatomic, retain) NNWBrowserViewController *browserViewController;
@property (nonatomic, retain) NSString *listControllerKey;
@property (nonatomic, retain) NSViewController *detailTemporaryViewController;
@property (nonatomic, retain) RSArticleListController *articleListController;
@property (nonatomic, retain) RSDataArticle *contextualMenuArticle;
@property (nonatomic, retain) RSMacWebView *webView;
@property (nonatomic, retain, readonly) RSDataArticle *selectedArticle;
...
- (void)dealloc {
	...
	[articleDetailPaneView release];
	[articleListController release];
	[articleListScrollView release];
	[articles release];
	[browserViewController release];
	[currentWebView release];
	[detailTemporaryViewController release];
	[feeds release];
	[listControllerKey release];
	[rightPaneContainerView release];
	[selectedArticles release];
...
- (void)awakeFromNib {
	...
	self.articleListController = [[[RSArticleListController alloc] init] autorelease];
...
- (NSView *)listView:(id)listView groupViewForRow:(NSUInteger)row {
	...
	if (aView == nil)
		aView = [[[NNWArticleGroupHeaderView alloc] initWithFrame:groupRowFrame] autorelease];
...
- (NSView *)listView:(id)listView viewForRow:(NSUInteger)row {
	...
	if (aView == nil)
		aView = [[[NNWArticleListView alloc] init] autorelease];
	...
	for (RSDataArticle *oneArticle in someArticles) {
		...
		if (oneGroup == RSDateGroupPast) {
			...
			if (year != lastYear || month != lastMonth) {
				NNWArticleListGroupItem *yearMonthGroupItem = [[[NNWArticleListGroupItem alloc] init] autorelease];
		...
		NNWArticleListGroupItem *groupItem = [[[NNWArticleListGroupItem alloc] init] autorelease];
...
- (void)updateSortOrder {
	RSDataArticle *currentArticle = [[self.selectedArticle retain] autorelease];
	self.sortAscending = [[NSUserDefaults standardUserDefaults] boolForKey:@"sortArticlesOldestAtTop"];
	[self updateArticles];
	if (currentArticle == nil)
		return;
...
- (void)loadEmptyArticleInDetailView {
	...
	NNWHTMLBuilderArticle *htmlBuilderArticle = [[[NNWHTMLBuilderArticle alloc] initWithArticle:nil htmlTemplate:articleTheme.htmlTemplate styleSheetPath:styleSheetURLString] autorelease];
...
- (void)loadHTMLInDetailView {
	...
	NNWHTMLBuilderArticle *htmlBuilderArticle = [[[NNWHTMLBuilderArticle alloc] initWithArticle:articleToDisplay htmlTemplate:articleTheme.htmlTemplate styleSheetPath:styleSheetURLString] autorelease];
	htmlBuilderArticle.includeHTMLHeader = YES;
	htmlBuilderArticle.includeHTMLFooter = YES;
...
- (NSView *)detailContentViewForSelectedArticles {
	...
	if (self.webView == nil) {
		self.webView = [[[RSMacWebView alloc] initWithFrame:[self.articleDetailPaneView frame] frameName:nil groupName:nil] autorelease];
...
- (void)openURLInInternalBrowser:(NSURL *)aURL {
	...
	self.browserViewController = [[[NNWBrowserViewController alloc] init] autorelease];
...
- (void)refreshSessionDidEnd:(NSNotification *)note {
	...
	if (RSIsEmpty(self.feeds))
		return;
	NSArray *currentArticles = [[self.selectedArticles retain] autorelease];
	self.articleListController.sortAscending = self.sortAscending;
	self.articleListController.feedsAndFolders = self.feeds;
	...
	[articles autorelease];
	articles = [updatedArticles retain];
...
- (NSArray *)webView:(WebView *)sender contextMenuItemsForElement:(NSDictionary *)element defaultMenuItems:(NSArray *)defaultMenuItems {
	...
	NSMutableArray *filteredItems = [[defaultMenuItems mutableCopy] autorelease];
	...
	NSMenu *browserContextualMenu = [[[NSMenu alloc] initWithTitle:@"Browser Contextual Menu for Links"] autorelease];
	...
	NSArray *items = [[[browserContextualMenu itemArray] copy] autorelease];
...
- (NSMenu *)contextualMenuForArticle:(RSDataArticle *)article {
	...
	NSMenu *menu = [[[NSMenu alloc] initWithTitle:@"Article Contextual Menu"] autorelease];
