...
@property (nonatomic, retain) NSMutableSet *checkedURLs; //so we don't keep re-trying unsuccessful URLs
...
- (void)downloadOperationDidFinish:(RSDownloadOperation *)downloadOperation {
	...
	NSInvocationOperation *saveThumbnailOperation = [[[NSInvocationOperation alloc] initWithTarget:self selector:@selector(saveThumbnailWithDictionary:) object:thumbnailDictionary] autorelease];
...
- (void)downloadImageAtURL:(NSURL *)imageURL {
	RSDownloadOperation *downloadOperation = [[[RSDownloadOperation alloc] initWithURL:imageURL delegate:self callbackSelector:@selector(downloadOperationDidFinish:) parser:nil useWebCache:YES] autorelease];
...
- (CGImageRef)thumbnailForURL:(NSURL *)aURL {
	...
	if (cgImage != nil)
		return cgImage;
	NSInvocationOperation *fetchThumbnailOperation = [[[NSInvocationOperation alloc] initWithTarget:self selector:@selector(fetchThumbnail:) object:aURL] autorelease];
