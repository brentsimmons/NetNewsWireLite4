...
@property (nonatomic, retain) NSString *mouseOverURL;
@property (nonatomic, retain) NSString *selectedURL;
@property (nonatomic, retain) NSString *statusBarURL;
@property (nonatomic, retain) id<RSSharableItem> presentedSharableItem;
@property (nonatomic, retain, readwrite) NSString *toggleReadMenuItemTitle;
...
- (id)init {
	...
	if (self == nil)
		return nil;
	toggleReadMenuItemTitle = [NNW_MARK_READ retain];
	return self;
}
...
- (void)dealloc {
	...
	[articleListDelegate release];
	[dataController release];
	[mouseOverURL release];
	[pluginManager release];
	[presentedSharableItem release];
	[selectedURL release];
	[sourceListDelegate release];
	[sourceListTreeBuilder release];
	[statusBarURL release];
	[toggleReadMenuItemTitle release];
...
- (void)addPluginCommandsInPlugin:(id<RSPlugin>)aPlugin toMenu:(NSMenu *)menu withSharableItem:(id<RSSharableItem>)sharableItem {
	...
	for (id<RSPluginCommand> oneCommand in aPlugin.allCommands) {
		...
		if ([self validateSharingPluginCommand:oneCommand withSharableItem:sharableItem]) {
			...
			NSMenuItem *menuItem = [[[NSMenuItem alloc] initWithTitle:oneCommand.title action:@selector(performSharingPluginCommandWithAssociatedObject:) keyEquivalent:@""] autorelease];
