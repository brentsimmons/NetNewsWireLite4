...
//    NSString *folder = RSCacheFolderForAppSubFolder(RSWebClipIconCacheFolderName, YES);
    ...
    self.imageFolderCache = [[RSImageFolderCache alloc] initWithFolder:folder];
...
- (void)downloadWebClipIconAtURL:(NSURL *)webclipIconURL {
    RSDownloadOperation *webclipIconDownloadOperation = [[RSDownloadOperation alloc] initWithURL:webclipIconURL delegate:self callbackSelector:@selector(webclipIconDownloadOperationDidFinish:) parser:nil useWebCache:NO];
...
- (CGImageRef)webclipIconForURL:(NSURL *)webclipIconURL {
    ...
    if (cgImage != nil)
        return cgImage;
    NSInvocationOperation *fetchIconOperation = [[NSInvocationOperation alloc] initWithTarget:self selector:@selector(fetchIcon:) object:webclipIconURL];
