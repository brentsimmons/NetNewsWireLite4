...
@property (nonatomic, retain) RSTreeNode *localAccountNode;
...
- (id)initWithDataController:(RSDataController *)aDataController {
	...
	if (self == nil)
		return nil;
	dataController = [aDataController retain];
	gMyInstance = [self retain];
	return self;
}
...
- (void)rebuildTree {
	self.tree = [[[RSTree alloc] init] autorelease];
//	[self addGlobalNodes];
	[self addAccounts];
}
...
- (void)invalidateAllCaches {
	NSArray *flatItems = [[self.tree.flatItems copy] autorelease];
