...
@property (nonatomic, retain) NSCache *styleSheetCache;
@property (nonatomic, retain) NSString *folderPath;
@property (nonatomic, retain, readwrite) NSArray *styleSheetNames;
@property (nonatomic, retain, readwrite) NSString *installError;
...
- (id)init {
	...
	folderPath = [RSSubFolderInFolder(rs_app_delegate.pathToDataFolder, NNWStyleSheetFolderName, YES) retain];
...
- (void)dealloc {
	...
	[styleSheetCache release];
	[styleSheetNames release];
	[super dealloc];
}
...
- (NNWArticleTheme *)articleThemeWithName:(NSString *)articleThemeName {
	...
	if (RSFileExists(styleSheetPath)) {
		styleSheet = [[[NNWArticleTheme alloc] initWithFolderPath:styleSheetPath] autorelease];
