...
- (void)addCommandsFromPlistFileAtPath:(NSString *)plistFilePath toArray:(NSMutableArray *)anArray {
    ...
    for (NSString *oneKey in [sendToAppsDictionary allKeys]) {
        NNWSendToAppSpecifier *oneSendToAppSpecifier = [[NNWSendToAppSpecifier alloc] initWithAppName:oneKey configInfo:[sendToAppsDictionary objectForKey:oneKey]];
        if (oneSendToAppSpecifier.appExistsOnDisk && ![self commandForAppAlreadyExists:oneKey inArray:anArray])
            [anArray addObject:[[NNWPluginCommandSendToApp alloc] initWithAppSpecifier:oneSendToAppSpecifier]];
    }    
}
