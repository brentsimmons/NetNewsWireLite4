...
@property (nonatomic, retain) NSButton *unreadButton;
@property (nonatomic, retain) id thumbnail; //CGImageRef
@property (nonatomic, retain) id webclipIcon; //CGImageRef
...
- (id)initWithFrame:(NSRect)frameRect {
	...
	if (gBadThumbnailURLs == nil)
		gBadThumbnailURLs = [[NSMutableSet set] retain];
...
- (void)dealloc {
	...
	[article release];
	[thumbnail release];
	[logicalThumbnailURL release];
	[unreadButton release];
	[webclipIcon release];
	[super dealloc];
}
...
- (void)addUnreadButton {	
	...
	self.unreadButton = [[[NSButton alloc] initWithFrame:rButton] autorelease];
...
- (NSFont *)titleFont {
	...
	if (gTitleFont == nil)
	gTitleFont = [[NSFont boldSystemFontOfSize:kNNWArticleTitleFontSize] retain];
	return gTitleFont;
}
...
- (NSFont *)previewFont {
	...
	if (gPreviewFont == nil)
		gPreviewFont = [[NSFont systemFontOfSize:kNNWArticlePreviewFontSize] retain];
	return gPreviewFont;	
}
...
- (NSFont *)dateFont {
	...
	if (gDateFont == nil)
		gDateFont = [[NSFont boldSystemFontOfSize:kNNWArticleDateFontSize] retain];
	return gDateFont;
}
...
- (NSFont *)timeFont {
	...
	if (gTimeFont == nil)
		gTimeFont = [[NSFont systemFontOfSize:kNNWArticleDateFontSize] retain];
	return gTimeFont;	
}
...
- (NSFont *)feedNameFont {
	...
	if (gFeedNameFont == nil)
		gFeedNameFont = [[NSFont systemFontOfSize:kNNWArticleFeedNameFontSize] retain];
...
- (NSColor *)frameColor {
	...
	if (frameColor == nil)
		frameColor = [[NSColor colorWithDeviceRed:139.0f/255.0f green:146.0f/255.0f blue:157.0f/255.0f alpha:1.0f] retain];
	return frameColor;
}
...
- (void)drawRect:(NSRect)r {
	...
	if (lineHeight < 1.0f) {
		...
		NSAttributedString *lineHeightString = [[[NSAttributedString alloc] initWithString:@"KyZPjgQ" attributes:lineHeightAttributes] autorelease];
...
//	NSFrameRect(rTextBox);
	...
	static NSColor *previewColor = nil;
			if (previewColor == nil)
				previewColor = [[self frameColor] retain];
	...
	NSAttributedString *titleString = [[[NSAttributedString alloc] initWithString:(self.title ? self.title : @"") attributes:attributes] autorelease];
	...
	if (rPreview.size.height > 1.0f && CGRectIntersectsRect(rPreview, r)) {
		...
		if (!RSStringIsEmpty(preview)) {
			...
			NSAttributedString *previewString = [[[NSAttributedString alloc] initWithString:preview attributes:previewAttributes] autorelease];			
	...
	if (dateColor == nil)
		dateColor = [[NSColor colorWithDeviceRed:157.0f/255.0f green:194.0f/255.0f blue:235.0f/255.0f alpha:1.0f] retain];
	static NSColor *dateReadColor = nil;
	if (dateReadColor == nil)
		dateReadColor = [dateColor retain];
	...
	if (date != nil) {
		...
		RSDateGroup dateGroup = [[RSDateManager sharedManager] groupForDate:date];
			NSShadow *dateShadow = [[[NSShadow alloc] init] autorelease];
		...
		if (shouldShowTime) {
			...
			NSMutableAttributedString *fullDateString = [[[NSMutableAttributedString alloc] initWithString:dateAndTimeString attributes:dateAttributes] autorelease];
