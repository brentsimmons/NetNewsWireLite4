...
- (id)initWithAppName:(NSString *)anAppName configInfo:(id)configInfo {
    ...
    if ([configInfo isKindOfClass:[NSString class]]) {
        appBundleID = configInfo;
        usesAPI = YES;
    }
    
    else if ([configInfo isKindOfClass:[NSDictionary class]]) {
        appBundleID = [configInfo objectForKey:NNWSendToAppBundleIDKey];
        URLSchemeTemplate = [configInfo objectForKey:NNWSendToAppURLTemplateKey];
...
- (NSImage *)icon {
    ...
    if (self.appPath == nil)
        return nil;
    icon = [[NSWorkspace sharedWorkspace] iconForFile:self.appPath];
    return icon;
}
...
@property (nonatomic, strong) NNWSendToAppSpecifier *sendToAppSpecifier;
@property (nonatomic, strong) id<RSSharableItem> sharableItem;
...
- (id)initWithAppSpecifier:(NNWSendToAppSpecifier *)aSendToAppSpecifier {
    ...
    if (self == nil)
        return nil;
    sendToAppSpecifier = aSendToAppSpecifier;
    return self;
}
...
static NSAppleEventDescriptor *targetDescriptorForRunningAppAtPath(NSString *appPath) {
    ...
    while (true) {
        ...
        if (err != noErr)
            break;
        NSDictionary *infoDict = (NSDictionary *)CFBridgingRelease(ProcessInformationCopyDictionary(&psn, (UInt32)kProcessDictionaryIncludeAllInformationMask));
    ...
    if (foundPSN)
        return [[NSAppleEventDescriptor alloc] initWithDescriptorType:typeProcessSerialNumber data:[NSData dataWithBytes:&psn length:sizeof(ProcessSerialNumber)]];    
...
- (BOOL)sendUsingURLScheme {
    ...
    NSMutableString *urlStringToSend = [self.sendToAppSpecifier.URLSchemeTemplate mutableCopy];
