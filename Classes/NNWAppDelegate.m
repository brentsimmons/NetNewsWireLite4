...
@property (nonatomic, strong) NNWAddFeedWithURLWindowController *addFeedWindowController;
@property (nonatomic, strong) NNWAddFolderWindowController *addFolderWindowController;
@property (nonatomic, strong) NNWDockIconController *dockIconController;
@property (nonatomic, strong) NNWMainWindowController *mainWindowController;
@property (nonatomic, strong) NNWSourceListTreeBuilder *sourceListTreeBuilder;
@property (nonatomic, strong) NNWStyleSheetController *styleSheetController;
@property (nonatomic, strong) NSDate *lastRefreshDate;
@property (nonatomic, strong) NSTimer *periodicRefreshTimer;
@property (nonatomic, strong) RSContainerWithTableWindowController *addFeedsWindowController;
@property (nonatomic, strong) RSFaviconController *faviconController;
@property (nonatomic, strong) RSLocalAccountFeedMetadataCache *localAccountFeedMetadataCache;
@property (nonatomic, strong) RSOperationController *mainOperationController;
@property (nonatomic, strong) id<RSSharableItem> presentedSharableItem;
@property (nonatomic, strong, readwrite) NNWRefreshController *refreshController;
@property (nonatomic, strong, readwrite) NSArray *sharingPluginsWithGroupedCommands;
@property (nonatomic, strong, readwrite) NSArray *sharingPluginsWithSoloCommands;
@property (nonatomic, strong, readwrite) NSString *applicationNameForWebviewUserAgent;
@property (nonatomic, strong, readwrite) NSString *pathToCacheFolder;
@property (nonatomic, strong, readwrite) NSString *pathToDataFolder;
@property (nonatomic, strong, readwrite) NSString *pathToTemporaryCacheFolder;
@property (nonatomic, strong, readwrite) NSString *userAgent;
@property (nonatomic, strong, readwrite) RSDataController *dataController;
@property (nonatomic, strong, readwrite) RSPluginManager *pluginManager;
...
- (id)init {
    ...
    pathToDataFolder = appSupportFolder;
...
//#endif
    ...
    self.userAgent = [NSString stringWithFormat:@"NetNewsWire/%@ (Mac OS X; http://netnewswireapp.com/mac/; gzip-happy)", [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleShortVersionString"]];        
    ...
    self.sourceListTreeBuilder = [[NNWSourceListTreeBuilder alloc] initWithDataController:self.dataController];
    ...
    self.refreshController = [[NNWRefreshController alloc] init];    
    [self performSelectorOnMainThread:@selector(refreshAll:) withObject:nil waitUntilDone:NO];

    self.mainWindowController = [[NNWMainWindowController alloc] init];
...
- (void)setupDataController {
    self.dataController = [[RSDataController alloc] initWithModelResourceName:RSCoreDataModelResourceName storeFileName:RSCoreDataStoreFileName];
...
- (NSMenuItem *)addSharingCommand:(id<RSPluginCommand>)pluginCommand toMenu:(NSMenu *)aMenu keyEquivalent:(NSString *)keyEquivalent indentationLevel:(NSInteger)indentationLevel includeKeyboardShortcuts:(BOOL)includeKeyboardShortcuts {
    ...
    NSMenuItem *menuItem = [[NSMenuItem alloc] initWithTitle:title action:@selector(performSharingPluginCommandWithSender:) keyEquivalent:keyEquivalent];
...
- (NSArray *)orderedSharingPluginCommands {
    ...
    orderedSharingPluginCommands = tempArray;
    return orderedSharingPluginCommands;
}
...
- (void)updateMenuWithPlugins {
    ...
    NSMenuItem *openInBrowserMenuItem = [[NSMenuItem alloc] initWithTitle:openInBrowserCommand.title action:@selector(performSharingPluginCommandWithSender:) keyEquivalent:@"b"];
...
- (void)addFeedWithURLString:(NSString *)urlString { //urlString may be nil
    if (self.runningModalSheet)
        return;
    self.addFeedWindowController = [[NNWAddFeedWithURLWindowController alloc] initWithURLString:urlString];
...
- (void)addFeedWithSubscribeRequest:(NNWSubscribeRequest *)aSubscribeRequest {
    if (self.runningModalSheet)
        return;
    self.addFeedWindowController = [[NNWAddFeedWithURLWindowController alloc] initWithSubscribeRequest:aSubscribeRequest];
    [self runModalSheet:[self.addFeedWindowController window]];        
}
...
- (void)getURL:(NSAppleEventDescriptor *)event withReplyEvent:(NSAppleEventDescriptor *)reply {
    ...
    NSString *urlString = [rawURLString copy];
...
- (void)makeDefaultAggregator {
    LSSetDefaultHandlerForURLScheme(CFSTR("feed"), (__bridge CFStringRef)[[NSBundle mainBundle] bundleIdentifier]);
}
...
- (void)addFolder:(id)sender {
    self.addFolderWindowController = [[NNWAddFolderWindowController alloc] init];
...
- (void)openPreferencesWindow:(id)sender {
    ...
    if (preferencesWindowController == nil) {
        ...
        for (NSString *onePluginClassName in builtInPreferencesPluginClassNames)
            [preferencesPlugins addObject:[[NSClassFromString(onePluginClassName) alloc] init]];
