...
- (BOOL)shouldRegister:(id<RSPluginManager>)pluginManager {
	self.allCommands = [NSArray arrayWithObject:[[[NNWSharingCommandOpenInBrowser alloc] init] autorelease]];
	return YES;
}
...
- (void)openURL:(NSURL *)url inBackground:(BOOL)inBackground {
	...
	if (inBackground) {
		...
		urlSpec.appURL = nil;	
		urlSpec.itemURLs = (CFArrayRef)[NSArray arrayWithObject:massagedURL];
