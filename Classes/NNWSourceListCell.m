...
- (void)drawWithFrame:(NSRect)cellFrame inView:(NSView *)controlView {
	...
	if (self.countForDisplay > 0) {
		...
		if (wNine < 1) {
			NSAttributedString *nineString = [[[NSAttributedString alloc] initWithString:@"9" attributes:[NSDictionary dictionaryWithObjectsAndKeys:boldFont, NSFontAttributeName, nil]] autorelease];
			wNine = (NSUInteger)[nineString size].width;
		}
		NSUInteger wUnread = (ctDigits * wNine);//; - 1;// + 15;
		static NSBezierPath *pUnread = nil;
		if (!pUnread) {
			pUnread = [[NSBezierPath bezierPath] retain];
		...
		if (unreadChicletColor == nil)
			...
			unreadChicletColor = [[[NSColor colorWithDeviceRed:139.0f/255.0f green:146.0f/255.0f blue:157.0f/255.0f alpha:1.0f] highlightWithLevel:0.4f] retain];
		...
		NSAttributedString *unreadString = [[[NSAttributedString alloc] initWithString:tempString attributes:atts] autorelease];
