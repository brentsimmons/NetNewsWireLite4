...
@interface RSGoogleSubsListParser ()
...
- (void)dealloc {
	[RSParsedGoogleCategory emptyCategoryCache];
	[subs release];
...
- (void)addSub {
	[self.subs addObject:[[[RSParsedGoogleSub alloc] init] autorelease]];
}


- (RSParsedGoogleSub *)currentSub {
...
- (void)xmlEndElement:(const xmlChar *)localName prefix:(const xmlChar *)prefix uri:(const xmlChar *)uri {
	...
	if (tagSpecifier == RSGoogleSubsObject && self.inCategories) {
		if (!RSStringIsEmpty(self.lastCategoryID)) {
			RSParsedGoogleCategory *category = [RSParsedGoogleCategory categoryWithGoogleID:self.lastCategoryID];
