...
+ (BOOL)guidIsFromGoogleReader:(NSString *)guid {
	if ([guid hasPrefix:NNWPrefixForGoogleReaderGuid] && [guid caseSensitiveContains:NNWGoogleReaderGuidMustContain])
		return YES;
	return [guid length] == 16; //short item id
}
...
- (id)init {
	...
	[[self database] executeUpdate:@"CREATE INDEX if not exists readStatusIndex on readActions (status);"];
	[[self database] executeUpdate:@"CREATE INDEX if not exists starStatusIndex on starActions (status);"];
...
- (NSSet *)pendingStarredShortItemIDs:(BOOL)starred {
	NSMutableSet *starredItemIDs = [NSMutableSet set];
	NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
	@synchronized(self) {
		FMResultSet *rs = [[self database] executeQuery:@"select googleItemID from starActions where status = ?;", [NSNumber numberWithBool:starred]];
		while (rs && [rs next]) {
			NSString *oneItemID = [rs stringForColumn:0];
			if (RSStringIsEmpty(oneItemID))
				continue;
			[starredItemIDs rs_addObject:NNWGoogleShortItemIDForLongItemID(oneItemID)];
		}
		[rs close];
	}
	[pool drain];
...
- (void)_sendNextChangesToGoogle {
	...
	if (!didSendOneChange) { /*Star/unstar*/
		...
		if (RSIsEmpty(change))
			self.hasPendingMarkStarredChanges = NO;
		else {
	...
	if (sendChangeDidSucceed && hasAnyPendingChanges)
		[self performSelector:@selector(_sendNextChangesToGoogle) withObject:nil afterDelay:0.01];
