...
+ (NNWHTTPResponse *)responseWithURLResponse:(NSURLResponse *)urlResponse data:(NSData *)data error:(NSError *)error {
	...
	if (error && [error code] == NSURLErrorNotConnectedToInternet)
		[[NSNotificationCenter defaultCenter] performSelectorOnMainThread:@selector(postNotification:) withObject:[NSNotification notificationWithName:BCNotConnectedToInternetNotification object:nil] waitUntilDone:NO];
	else if (!error)
		[[NSNotificationCenter defaultCenter] performSelectorOnMainThread:@selector(postNotification:) withObject:[NSNotification notificationWithName:BCConnectedToInternetNotification object:nil] waitUntilDone:NO];
